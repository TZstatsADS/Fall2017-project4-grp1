...
load('../output/spearman_weight.RData')
load('../output/spearman_weight_MS.RData')
load('../output/vector_weight.RData')
load('../output/vector_weight_MS.RData')
```
...
#thresholding = 0.1
neighbors.select <- function(simweights, thresholding = 0.0025){
...
results_simrank <- neighbors.select(user_sim)
```

```{r}
neighbors_pearson_movie <- neighbors.select(pearson_weight)
neighbors_spearman_movie <- neighbors.select(spearman_weight)
neighbors_vector_movie <- neighbors.select(vector_weight)

neighbors_pearson_MS <- neighbors.select(pearson_weight_MS)
neighbors_spearman_MS <- neighbors.select(spearman_weight_MS)
neighbors_vector_MS <- neighbors.select(vector_weight_MS)
...
pred.matrix <- function(testdat, traindat, simweights, top.neighbors){
 ...
 # neighbor.weights = try[user,top.neighbors]
  for(i in 1:nrow(traindat)){
      ...
      prediction.matrix[i,!test.loc[i,]] = round(avg.ratings[i] + ifelse(length(top.neighbors[[i]]) <2,(neighbor.ratings-neighbor.avg[top.neighbors[[i]]]) * neighbor.weights, apply((neighbor.ratings-neighbor.avg[top.neighbors[[i]]]) * neighbor.weights, 2, sum, na.rm=T)) / sum(neighbor.weights, na.rm = T), 0)
...
pearsonpred = pred.matrix(movie_test,movie_train,pearson_weight,results)
simrankpred = pred.matrix(movie_test,movie_train,user_sim,results_simrank)
```

```{r}

pred_pearson_movie <- pred.matrix(movie_test,movie_train,pearson_weight,neighbors_pearson_movie)
pred_spearman_movie <- pred.matrix(movie_test,movie_train,spearman_weight,neighbors_spearman_movie)
pred_vector_movie <- pred.matrix(movie_test,movie_train,vecotr_weight,neighbors_vecotr_movie)

pred_pearson_MS <- pred.matrix(MS_test,MS_train,pearson_weight_MS,neighbors_pearson_MS)
pred_spearman_MS <- pred.matrix(MS_test,MS_train,spearman_weight_MS,neighbors_spearman_MS)
pred_vector_MS <- pred.matrix(MS_test,MS_train,vecotr_weight_MS,neighbors_vecotr_MS)
...
mae.simraank = evaluation.mae(simrankpred, movie_test)
```
...
## Combine 
```{r}
calculate <- function(data,SimWeight,SigWeight = F,VarWeight = F,EvaluateMethod){
  ## input:
  ## data - 'movie' or 'MS'
  ## SimWeight - 'pearson' or 'spearman' or 'vector' or 'simrank'
  ## SigWeight - T or F
  ## VarWeight - T or F
  ## EvaluateMethod - 'MAE' or 'ROC' or 'rank'
  
  ##### data
  if(data=='movie'){
    train_data <- movie_train
    test_data <- movie_test
  }else if(data=='MS'){
    train_data <- MS_train
    test_data <- MS_test
  }
