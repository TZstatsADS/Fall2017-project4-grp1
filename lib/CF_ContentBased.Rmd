...
#try: weights Table
pred.matrix <- function(testdat, try, top.neighbors){
  prediction.matrix = matrix(NA, ncol = ncol(testdat), nrow = nrow(testdat))
  avg.ratings = apply(testdat, 1, mean, na.rm = T)
 # neighbor.weights = try[user,top.neighbors]
  for(i in 1:nrow(testdat)){
      neighbor.weights = try[i,top.neighbors[i,]]
      neighbor.ratings = testdat[top.neighbors[i,], ]
      neighbor.avg = apply(testdat[top.neighbors[i,], ],1,mean,na.rm= T)
...
mae.pearson = evaluation.mae(pearsonpred, pearson_weight)
```
